...
class CodeGenerationCLI {
  ...
  async generateComponent(args) {
    ...
    const requirements = {
      name: name,
      description:
        description || `A ${name} component for the AI Finance Tracker`,
      props: [
        {
          name: 'className',
          type: 'string',
    ...
    // Save types file
    fs.writeFileSync(path.join(componentDir, 'types.ts'), generated.types);
  ...
  async generateHook(args) {
      ...
      parameters: [
        {
          name: 'initialValue',
          type: 'T',
          description: 'Initial value for the hook',
        },
      ],
    ...
    // Save hook file
    fs.writeFileSync(path.join(hooksDir, `use${name}.ts`), generated.hook);
  ...
  async generateAPI(args) {
    ...
    // Save endpoint file
    fs.writeFileSync(path.join(apiDir, 'route.ts'), generated.endpoint);

    // Save types file
    fs.writeFileSync(path.join(apiDir, 'types.ts'), generated.types);

    // Save tests file
    fs.writeFileSync(path.join(apiDir, 'route.test.ts'), generated.tests);
  ...
  async generateModel(args) {
    ...
    const requirements = {
      name: name,
      description:
        description || `A ${name} model for the AI Finance Tracker database`,
      fields: [
