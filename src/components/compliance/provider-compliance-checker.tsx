...
import { ComplianceManager, ProviderName, UserTier } from '@/lib/compliance/provider-requirements';
...
export function ProviderComplianceChecker({
  ...
  const requirements = ComplianceManager.getProviderRequirements(selectedProvider);
  const transferLimits = ComplianceManager.getTransferLimits(selectedProvider, userTier);
  const supportedCurrencies = ComplianceManager.getSupportedCurrencies(selectedProvider);
  const complianceStandards = ComplianceManager.getComplianceStandards(selectedProvider);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Badge variant={requirements.businessRegistration ? 'destructive' : 'default'}>
                  {requirements.businessRegistration ? 'Required' : 'Not Required'}
                </Badge>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <Badge variant={
                  requirements.kycLevel === 'premium' ? 'destructive' :
                  requirements.kycLevel === 'enhanced' ? 'default' : 'secondary'
                }>
...
export function ProviderSelector({
  ...
  const providers = Object.keys(ComplianceManager.getComplianceSummary()) as ProviderName[];
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {providers.map(provider => {
            const requirements = ComplianceManager.getProviderRequirements(provider);
            const transferLimits = ComplianceManager.getTransferLimits(provider, userTier);
              ...
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={requirements?.businessRegistration ? 'destructive' : 'default'}>
                      {requirements?.businessRegistration ? 'Business Required' : 'Personal OK'}
                    </Badge>
                  </div>
                  <p className="text-sm text-gray-600">{requirements?.region}</p>
                  <div className="flex space-x-1">
                    ...
                    {requirements?.currencies.length > 3 && (
                      <Badge variant="outline" className="text-xs">
                        +{requirements.currencies.length - 3}
                      </Badge>
                    )}
