...
export function BudgetOverview({ utilization }: BudgetOverviewProps) {
  const budgetStatus = utilization > 90 ? 'over' : utilization > 75 ? 'warning' : 'good'
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span>Spent</span>
            <span className="font-medium">${(5000 * utilization / 100).toFixed(0)}</span>
          </div>
          <Progress value={utilization} className="h-2" />
          <div className="flex justify-between text-sm">
            <span>Remaining</span>
            <span className="font-medium">${(5000 * (100 - utilization) / 100).toFixed(0)}</span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {[
              ...
              { name: 'Transportation', spent: 450, budget: 600, percentage: 75 },
              { name: 'Entertainment', spent: 300, budget: 400, percentage: 75 },
            ].map((category) => (
