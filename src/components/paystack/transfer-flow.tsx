...
export function PaystackTransferFlow() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [recipientsRes, transfersRes, banksRes, accountsRes] =
        await Promise.all([
    ...
    <div className='max-w-6xl mx-auto p-6'>
      ...
      <div className='mb-8'>
        ...
        <p className='text-muted-foreground'>
          Complete transfer flow: Create recipient → Initiate transfer → Webhook
          handling
        </p>
      </div>
      ...
      >
        ...
        <TabsContent value='transfers' className='space-y-6'>
          <TransfersHistory transfers={transfers} onRefresh={loadData} />
        </TabsContent>

        <TabsContent value='verify' className='space-y-6'>
          <TransferVerification onVerifySuccess={loadData} />
        </TabsContent>
      </Tabs>
    </div>
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Transfer initiated: ${data.data.paystack.reference}`);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Send money to a saved recipient</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              >
                ...
                <SelectContent>
                  {recipients.map(recipient => (
                    <SelectItem
                      key={recipient.id}
                      value={recipient.paystackRecipientId}
                    >
  ...
  const handleAddRecipient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        toast.error(error.error || 'Failed to create recipient');
      }
    } catch (error) {
                ...
                <div className='flex items-center space-x-2'>
                  ...
                  {recipient.lastUsedAt && (
                    <span className='text-xs text-gray-500'>
                      Last used:{' '}
                      {new Date(recipient.lastUsedAt).toLocaleDateString()}
                    </span>
                  )}
                </div>
              </div>
            ...
            {recipients.length === 0 && (
              <div className='text-center py-8'>
                ...
                <p className='text-sm text-gray-400'>
                  Add a recipient to start making transfers
                </p>
              </div>
            )}
          </div>
              ...
              <div className='flex items-center space-x-4'>
                ...
                <div>
                  <h4 className='font-medium'>
                    Transfer to {transfer.recipient.name}
                  </h4>
                  <p className='text-sm text-gray-600'>
...
function TransferVerification({
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (!reference) {
      toast.error('Please enter a transfer reference');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setResult(data.data);
        toast.success('Transfer verification completed');
        onVerifySuccess();
      } else {
        const error = await response.json();
        toast.error(error.error || 'Failed to verify transfer');
      }
    } catch (error) {
