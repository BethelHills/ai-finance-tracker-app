...
export function useAIInsights(
  ...
  const { autoGenerate = false, refreshInterval = 0, onError } = options;
...
export function useTransactionCategorization() {
  ...
  const [categorizationHistory, setCategorizationHistory] = useState<
    Array<{
      id: string;
      description: string;
      amount: number;
  ...
  const getCategorizationAccuracy = useCallback(() => {
    ...
    const highConfidence = categorizationHistory.filter(
      c => c.confidence > 0.8
    ).length;
...
export function useBudgetOptimization() {
  ...
  const getTotalSavingsPotential = useCallback(() => {
    return optimizationHistory.reduce(
      (total, opt) => total + opt.savingsPotential,
      0
    );
  }, [optimizationHistory]);
