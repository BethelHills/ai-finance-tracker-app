...
import { apiClient, TransactionQuery, CreateTransactionInput, UpdateTransactionInput } from '@/lib/api-client'
...
export function useTransactions(query: TransactionQuery = {}) {
  const apiCall = useCallback(() => apiClient.getTransactions(query), [query])
...
export function useTransactionStats(startDate?: string, endDate?: string) {
  const apiCall = useCallback(() => apiClient.getTransactionStats(startDate, endDate), [startDate, endDate])
...
export function useAICategorization() {
  ...
  const categorizeTransaction = useCallback(async (description: string, amount: number) => {
    ...
    try {
      const result = await apiClient.categorizeTransaction(description, amount)
      return result
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Failed to categorize transaction')
      setError(error)
      throw error
    } finally {
...
export function useBudgetOptimization() {
  ...
  const optimizeBudget = useCallback(async (currentSpending: Record<string, number>, income: number, goals: string[]) => {
    setIsOptimizing(true)
    setError(null)

    try {
      const result = await apiClient.optimizeBudget(currentSpending, income, goals)
      return result
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Failed to optimize budget')
      setError(error)
      throw error
    } finally {
...
export function useTransactionSearch() {
  ...
  const searchTransactions = useCallback(async (query: string, limit: number = 10) => {
    if (!query.trim()) {
      setSearchResults([])
      return
    }
