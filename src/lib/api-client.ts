...
export class APIClient {
  ...
  async getTransactions(query: TransactionQuery = {}): Promise<PaginatedTransactionsResponse> {
    ...
    return this.request<PaginatedTransactionsResponse>(`/api/transactions?${params}`, {
      method: 'GET'
    }, true) // Use cache
  }
  ...
  async searchTransactions(query: string, limit: number = 10): Promise<TransactionResponse[]> {
    return this.request<TransactionResponse[]>(`/api/transactions/search?q=${encodeURIComponent(query)}&limit=${limit}`, {
      method: 'GET'
    })
  }

  async getTransactionStats(startDate?: string, endDate?: string): Promise<TransactionStatsResponse> {
    const params = new URLSearchParams()
    if (startDate) params.append('startDate', startDate)
    if (endDate) params.append('endDate', endDate)
    
    return this.request<TransactionStatsResponse>(`/api/transactions/stats?${params}`, {
      method: 'GET'
    }, true) // Use cache
  }

  // AI operations
  async categorizeTransaction(description: string, amount: number): Promise<CategorizationResponse> {
  ...
  async optimizeBudget(currentSpending: Record<string, number>, income: number, goals: string[]): Promise<BudgetOptimizationResponse> {
    return this.request<BudgetOptimizationResponse>('/api/ai/budget-recommendations', {
      method: 'POST',
